    ...
    <header className="bg-white border-b border-gray-200 px-6 py-6">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="w-10 h-10 bg-gradient-to-br from-gray-900 to-gray-700 rounded-lg flex items-center justify-center">
            <svg viewBox="0 0 64 64" className="w-6 h-6" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g transform="translate(32, 32)">
                <circle cx="0" cy="0" r="14" fill="none" stroke="#fb923c" strokeWidth="1" opacity="0.25"/>
                <circle cx="0" cy="0" r="10" fill="none" stroke="#fb923c" strokeWidth="1.2" opacity="0.4"/>
        ...
        <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${config.bgColor} ${config.borderColor}`}>
          <div className={`w-2 h-2 rounded-full ${config.color} ${socketStatus === 'connected' ? 'animate-pulse' : socketStatus === 'reconnecting' ? 'animate-bounce' : ''}`} />
          <div className="flex flex-col">
            <span className={`text-xs font-semibold ${config.textColor}`}>{config.label}</span>
            <span className={`text-xs ${config.textColor} opacity-75`}>{config.sublabel}</span>
          </div>
        </div>
      </div>
    </header>
